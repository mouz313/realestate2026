...
@section('title', 'Add Enquiry <span class="urdu">(نئی انکوائری)</span>')

@section('content')
<div class="page-header">
    <h3>Add Enquiry <span class="urdu">(نئی انکوائری)</span></h3>
        ...
        <form method="POST" action="{{ route('contacts.store') }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="lead_source" class="form-select">
                        @foreach($leadSources as $key => $label)
                            <option value="{{ $key }}" {{ old('lead_source', 'walk_in') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" name="budget_min" class="form-control" value="{{ old('budget_min') }}" min="0">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Budget Max (Rs) <span class="urdu">(زیادہ سے زیادہ)</span></label>
                    <input type="number" name="budget_max" class="form-control" value="{{ old('budget_max') }}" min="0">
                </div>
                <div class="col-12">
                ...
                <div class="col-12">
                    <button type="submit" class="btn btn-dark"><i class="ti ti-device-floppy"></i> Save Enquiry</button>
                </div>
            </div>
        </form>
    </div>
