...
@section('title', 'Edit Rental Record <span class="urdu">(ریکارڈ ترمیم)</span>')
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit Rental Record <span class="urdu">(ریکارڈ ترمیم)</span></li>
    </ol>
</nav>
@endsection
...
<div class="card">
    <div class="card-header">
        <h4><i class="ti ti-edit me-1"></i> Edit Rental Record <span class="urdu">(ریکارڈ ترمیم)</span></h4>
    </div>
    <form action="{{ route('rental-records.update', $rentalRecord) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="property_id" class="form-select @error('property_id') is-invalid @enderror" required>
                            ...
                            @foreach($properties as $property)
                                <option value="{{ $property->id }}" @selected(old('property_id', $rentalRecord->property_id) == $property->id)>{{ $property->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tenant_select" id="tenant_select" class="form-select">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" @selected(old('tenant_id', $rentalRecord->tenant_id) == $client->id)>{{ $client->name }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="tenant_id" id="tenant_id" value="{{ old('tenant_id', $rentalRecord->tenant_id) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="landlord_select" id="landlord_select" class="form-select">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" @selected(old('landlord_id', $rentalRecord->landlord_id) == $client->id)>{{ $client->name }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="landlord_id" id="landlord_id" value="{{ old('landlord_id', $rentalRecord->landlord_id) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="created_by" class="form-select @error('created_by') is-invalid @enderror">
                            ...
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}" @selected(old('created_by', $rentalRecord->created_by) == $agent->id)>{{ $agent->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" name="start_date" class="form-control @error('start_date') is-invalid @enderror" value="{{ old('start_date', $rentalRecord->start_date?->format('Y-m-d')) }}">
                        @error('start_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-3">
                        <label class="form-label">End Date <span class="urdu">(اختتام کی تاریخ)</span></label>
                        <input type="date" name="end_date" class="form-control @error('end_date') is-invalid @enderror" value="{{ old('end_date', $rentalRecord->end_date?->format('Y-m-d')) }}">
                        @error('end_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Duration (months) <span class="urdu">(دورانیہ)</span></label>
                        <input type="number" name="duration_months" class="form-control @error('duration_months') is-invalid @enderror" value="{{ old('duration_months', $rentalRecord->duration_months) }}" min="0">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="active" @selected(old('status', $rentalRecord->status) == 'active')>Active</option>
                            <option value="ended" @selected(old('status', $rentalRecord->status) == 'ended')>Ended</option>
                        </select>
                ...
                <div class="col-12">
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3">{{ old('notes', $rentalRecord->notes) }}</textarea>
        ...
        <div class="card-footer d-flex flex-wrap gap-2">
            <button type="submit" class="btn btn-dark"><i class="ti ti-device-floppy"></i> Update Record <span class="urdu">(اپڈیٹ)</span></button>
