...
@section('title', 'Edit Token <span class="urdu">(ٹوکن میں ترمیم)</span>')
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit Token <span class="urdu">(ٹوکن میں ترمیم)</span></li>
    </ol>
</nav>
@endsection
...
<div class="card">
    <div class="card-header d-flex flex-wrap gap-2">
        <h4><i class="ti ti-coin"></i> <span class="urdu">(ٹوکن میں ترمیم کریں)</span></h4>
    </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('deal_id') is-invalid @enderror" name="deal_id" required>
                            ...
                            @foreach($deals ?? [] as $deal)
                                <option value="{{ $deal->id }}" {{ old('deal_id', $token->deal_id) == $deal->id ? 'selected' : '' }}>{{ $deal->deal_number }} - {{ $deal->property->title ?? '' }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount', $token->amount) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('payment_method') is-invalid @enderror" name="payment_method">
                            ...
                            <option value="cash" {{ old('payment_method', $token->payment_method) == 'cash' ? 'selected' : '' }}>Cash</option>
                            <option value="bank_transfer" {{ old('payment_method', $token->payment_method) == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                            <option value="cheque" {{ old('payment_method', $token->payment_method) == 'cheque' ? 'selected' : '' }}>Cheque</option>
                            <option value="online" {{ old('payment_method', $token->payment_method) == 'online' ? 'selected' : '' }}>Online Payment</option>
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('reference_no') is-invalid @enderror" name="reference_no" value="{{ old('reference_no', $token->reference_no) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('received_date') is-invalid @enderror" name="received_date" value="{{ old('received_date', $token->received_date ? $token->received_date->format('Y-m-d') : '') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" name="status">
                            <option value="received" {{ old('status', $token->status) == 'received' ? 'selected' : '' }}>Received</option>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" name="notes" rows="2">{{ old('notes', $token->notes) }}</textarea>
