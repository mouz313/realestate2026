...
@section('title', 'Log Call <span class="urdu">(کال درج کریں)</span>')
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Log Call <span class="urdu">(کال درج کریں)</span></li>
    </ol>
</nav>
@endsection
...
<div class="card">
    <div class="card-header">
        <h4><i class="ti ti-phone-plus me-1"></i> Log New Call <span class="urdu">(نئی کال درج کریں)</span></h4>
    </div>
    <form action="{{ route('call-logs.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @php $cr = old('caller_role', ''); @endphp
                        <div class="d-flex flex-wrap gap-2">
                            @foreach([['seller','Seller','بیچنے والا'],['buyer','Buyer','خریدنے والا'],['rent','Rent','کرایہ'],['installment','Installment','قسط']] as $opt)
                                <input type="radio" class="btn-check" name="caller_role" id="cr_{{ $opt[0] }}" value="{{ $opt[0] }}" autocomplete="off" @checked($cr == $opt[0])>
                                <label class="btn btn-outline-primary btn-sm" for="cr_{{ $opt[0] }}">{{ $opt[1] }} <span class="urdu">({{ $opt[2] }})</span></label>
                            @endforeach
                            <input type="radio" class="btn-check" name="caller_role" id="cr_unknown" value="" autocomplete="off" @checked($cr == '')>
                            <label class="btn btn-outline-secondary btn-sm" for="cr_unknown">Unknown <span class="urdu">(نامعلوم)</span></label>
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="lead_source" class="form-select">
                            ...
                            @foreach($leadSources as $key => $label)
                                <option value="{{ $key }}" @selected(old('lead_source', 'phone_call') == $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" min="0" class="form-control" name="bedrooms" value="{{ old('bedrooms') }}">
                    </div>
                </div>
            </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control" name="call_datetime" value="{{ old('call_datetime') }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
        ...
        <div class="card-footer d-flex flex-wrap gap-2">
            <button type="submit" class="btn btn-dark"><i class="ti ti-device-floppy"></i> Save Call <span class="urdu">(کال محفوظ)</span></button>
